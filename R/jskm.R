...
jskm <- function(sfit,
  ...
  # Removes the legend:
  if (legend == FALSE) {
    p <- p + theme(legend.position.inside = "none")
  }
  ...
  if (is.null(cut.landmark)) {
    if (is.null(col.pal)) {
      ...
      p <- p + scale_color_manual(name = ystrataname, values = col.pal, labels = ystratlabs2)
  ...
  # Add median value
  if (med == TRUE & is.null(status.cmprsk) & is.null(cut.landmark)){
    if (length(levels(summary(sfit)$strata)) == 0) {
      median_time <- summary(sfit)$table["median"][[1]]
      if(!is.na(median_time))
        p <- p + annotate("segment", x = 0, xend = median_time, y = 0.5, yend = 0.5, linewidth = 0.3, linetype = "dashed") + annotate("segment", x = median_time, xend = median_time, y = 0, yend = 0.5, linewidth = 0.3, linetype = "dashed") 
    } else {
      for (i in 1:length(levels(summary(sfit)$strata))){
        median_time <- summary(sfit)$table[,"median"][[i]]
        if(!is.na(median_time)){
  ...
  if (table == TRUE) {
    ...
    data.table <- data.table +
      theme(legend.position.inside = "none") + xlab(NULL) + ylab(NULL)
  ...
  if (!is.null(theme) && theme == "nejm") {
    p2 <- p1 + coord_cartesian(ylim = nejm.infigure.ylim) + theme(
      legend.position.inside = "none", axis.title.x = element_blank(), axis.title.y = element_blank(),
